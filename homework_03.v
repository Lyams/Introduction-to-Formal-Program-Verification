...
:= fun _ _ z r => match z r with
    erefl => erefl end.

(** * Exercise: Transitivity *)
Definition eqext_trans :
  forall (f g h : A -> B), f =1 g -> g =1 h -> f =1 h
:= fun f g h f_g g_h a => match g_h a : g a = h a in _ = b return f a = b with
    | erefl => f_g a end.
...
Definition eq_compl :
  forall (f g : A -> B) (h : B -> C),
    f =1 g -> h \o f =1 h \o g
:= fun f g h f_g a => match f_g a : f a = g a 
    in _ = c return _ = h c with erefl => erefl end.
